...
class OrderAgent:
    ...
    def findCouriers(self, couriers, order):
        ...
        for courier in couriers:
            ...
            # если расстояние до заказа больше, чем расстояние доставки, то курьер не возьмется
            if (totalDuration <= courier.workingDay) and (courier.canOrder(order)) and (courier.benefitDistance(order)):
