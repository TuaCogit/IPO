...
class CompanyAgent:
    def __init__(self):
        ...
        self.numOrder: int=0 #номер последнего заказа
        self.numCourier: int = 0  # номер последнего курьера
    ...
    def printCouriers(self):
        myTable = PrettyTable(["Курьер", "Скорость", "Грузоподъемность", "Местоположение","Время работы","Осталось работать"])
    ...
    #удаление отработавших курьеров
    def delCouriersEndWork(self):
        for courier in self.couriers:
           if ((courier.workingDay-sum(courier.durationOrders))<=0):
               self.couriers.remove(courier)

    #удаление курьера с переносом его заказов в невыполненные
    def delCouriers(self, courier):
        for compl in self.completedOrders:
            if compl.courier == courier:
                self.orders.extend(compl.orders)
                self.print_orders()
                self.couriers.remove(courier)
                self.printCouriers()
                self.completedOrders.remove(compl)
    ...
    def startPlaner(self):
        #отсортированные заказы по стоимости
        self.prepareQueue()
    ...
    def startCycle(self):
        ...
        while len(self.orderQueue) > 0: #пока в очереди есть заказы
            ...
            if result: #если заказ запланирован
                ...
                #получаем текущего назначенного на заказ курьера
                ...
                #смена местоположения курьера на место доставки
                thisCourier.changeLocation()
                ...
                for compl in self.completedOrders:
                    if compl.courier==thisCourier:
                        ...
                        compl.addOrder(orderForPlanning.currentPlan.order)
                        #дистанции от курьера до заказа и до места назначения
                        compl.addStrDist(orderForPlanning.currentPlan.currentDistance,orderForPlanning.currentPlan.order.getOrderDistance())
                ...
                print(
                    ...
                    f" курьер на расстоянии от заказа:{(str(round(orderForPlanning.currentPlan.currentDistance, 2)))} км\n"
        ...
        self.orders.clear() #очистить список заказов
        #если есть невыполненные заказы
        if self.ordersNotComplited:
            print(f"\nНевыполненные заказы")
            self.print_notCompletedOrders()
            #заменить список заказов невыполенными
            self.orders = list(self.ordersNotComplited)
             #очистить список невыполненных заказов
            self.ordersNotComplited.clear()
        print(f"Итоговая прибыль: {round(totalProfit, 2)}")
        self.delCouriersEndWork() #удалить отработавших курьеров
        print(f"Доступные курьеры:")
        self.printCouriers() #вывод отавшихся курьеров
